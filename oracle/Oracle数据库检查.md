...
-- 更改自动统计信息收集的窗口属性
-- 如，要更改星期一维护窗口，使其从凌晨5点开始，请执行以下PL / SQL程序：
BEGIN 
...
-- 要设置最大运行时间，请将AUTO_TASK_MAX_RUN_TIME首选项设置为所需的秒数。
-- 以下示例将最大运行时间设置为10分钟：
EXEC DBMS_STATS.SET_GLOBAL_PREFS('AUTO_TASK_MAX_RUN_TIME','600');
-- 要设置频率，请将AUTO_TASK_INTERVAL首选项设置为所需的秒数。
-- 下面的示例将频率设置为8分钟：
EXEC DBMS_STATS.SET_GLOBAL_PREFS('AUTO_TASK_INTERVAL','240');

-- 高频任务不会在维护时段内启动，因此一次只能运行一个统计任务。您可以通过查询监视任务COL OPID FORMAT 9999
...
```sh
自动清理oracle的归档日志、alert日志、监听日志、审计日志脚本。
#crontab
