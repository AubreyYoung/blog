# JOB_QUEUE_PROCESSES设置为0对不同版本数据库的影响.html

You need to enable JavaScript to run this app.

![](https://47.100.29.40/highgo_admin/static/media/head.530901d0.png)

  杨光  |  退出

  * 知识库

    * ▢  新建文档
    * ▢  文档复查 (3)
    * ▢  文档查询
  * 待编区

文档详细

  知识库 文档详细

![noteattachment1][7f025a17697ee15eb3197c61326b203b]

069251304

JOB_QUEUE_PROCESSES设置为0对不同版本数据库的影响

目录

环境

文档用途

详细信息

相关文档

内部备注

附件

环境

系统平台：N/A

版本：N/A

文档用途

用于说明JOB_QUEUE_PROCESSES设置为0时，对不同版本数据库的影响。

  

详细信息

Oracle 10g和11gR1版本中，设置JOB_QUEUE_PROCESSES=0，只是禁用 DBMS_JOB
jobs，而对DBMS_SCHEDULER jobs没有影响；

从11gR2版本开始，设置JOB_QUEUE_PROCESSES=0，同时禁用DBMS_SCHEDULER和DBMS_JOB jobs。

  

  

相关文档

内部备注

参考文章：

JOB_QUEUE_PROCESSES Parameter, its Significance and Recommended value (文档 ID
2118028.1)

  

In 10g and 11gR1, setting JOB_QUEUE_PROCESSES to 0 caused only DBMS_JOB jobs
to not run, but DBMS_SCHEDULER jobs were unaffected and would still run.

Starting with Oracle Database 11g Release 2 (11.2), setting
JOB_QUEUE_PROCESSES to 0 causes both DBMS_SCHEDULER and DBMS_JOB jobs to not
run.

  

附件

验证码：

  换一张

输入验证码：

![](data:image/png;base64,)![](data:image/png;base64,)

sentinel


---
### ATTACHMENTS
[7f025a17697ee15eb3197c61326b203b]: media/JOB_QUEUE_PROCESSES设置为0对不同版本数据库的影响.html
[JOB_QUEUE_PROCESSES设置为0对不同版本数据库的影响.html](media/JOB_QUEUE_PROCESSES设置为0对不同版本数据库的影响.html)
>hash: 7f025a17697ee15eb3197c61326b203b  
>source-url: file://C:\Users\galaxy\Desktop\highgo\JOB_QUEUE_PROCESSES设置为0对不同版本数据库的影响_files\11947ec6e06f4dd6a765bcc48c4fc88d.png  

---
### NOTE ATTRIBUTES
>Created Date: 2018-09-25 03:29:03  
>Last Evernote Update Date: 2018-10-01 15:33:55  
>author: YangKwong  
>source: desktop.win  
>source-url: file://C:\Users\galaxy\Desktop\highgo\JOB_QUEUE_PROCESSES设置为0对不同版本数据库的影响.html  
>source-application: evernote.win32  