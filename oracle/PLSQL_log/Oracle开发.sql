...
SELECT LOG_DATE, STATUS, WINDOW_NAME, OPERATION
  FROM DBA_SCHEDULER_WINDOW_LOG
 ORDER BY LOG_DATE DESC;

SELECT WINDOW_GROUP_NAME, ENABLED, NEXT_START_DATE
...
SELECT V4.EMPNO, V4.ENAME, B.EMPNO, B.ENAME
  FROM V4
  FULL JOIN EMP B
    ON (B.EMPNO = V4.EMPNO)
...
SELECT V.EMPNO, V.ENAME, V.CNT, EMP.EMPNO, EMP.ENAME, EMP.CNT
  FROM (SELECT EMPNO, ENAME, COUNT(*) AS CNT FROM V4 GROUP BY EMPNO, ENAME) V
  FULL JOIN (SELECT EMPNO, ENAME, COUNT(*) AS CNT
               FROM EMP
              GROUP BY EMPNO, ENAME) EMP
    ON (EMP.EMPNO = V.EMPNO AND V.CNT = EMP.CNT)
...
CREATE TABLE TEST1(C1 VARCHAR2(10) DEFAULT '默认1',
...
VALUES
  ('周五', D5)
  SELECT D1, D2, D3, D4, D5 FROM TEST3;
SELECT * FROM TEST3;
SELECT * FROM TEST4;
